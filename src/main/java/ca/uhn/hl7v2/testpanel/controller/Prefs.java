...
public class Prefs
{
  ...
  private static final Preferences ourPrefs
     = Preferences.userNodeForPackage(ca.uhn.hl7v2.testpanel.controller.Prefs.class);
  ...
  private void initFromJavaPrefs()
  {
    ...
    myMostRecentConnectionCharset
       = ourPrefs.get(GET_MOST_RECENT_CONNECTION_CHARSET, CharsetUtils.DEFAULT_CONNECTION_CHARSET.displayName());
    ...
    ArrayList<String> recentMessageXmlFilesSplit
  ...
  public static File getTempWorkfilesDirectory()
     throws IOException
  {
    File testPanelHome = getTestpanelHomeDirectory();
  ...
  public static File getTestpanelLogDirectory()
  {
    File rv = new File(homeDir, "logs");

    if(!rv.exists())
      if(!rv.mkdirs())
        throw new RuntimeException("The working directory could not be created: " + rv.getAbsolutePath());

    return rv;
  }

  public static File getTestpanelLogFile()
  {
    return new File(getTestpanelLogDirectory(), "testpanel.log");
  }

  private static final File homeDir = OsIdent.getAppDirectory(App.APP_NAME);
}
