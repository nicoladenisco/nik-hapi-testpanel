...
import org.commonlib5.gui.FileDialogHelper;
...
public class Hl7V2FileSortDialog extends JDialog
{
  ...
  private File lastUsedCurrentDir = new File(".");
  ...
  public Hl7V2FileSortDialog(Hl7V2FileSortController theHl7v2FileSortController)
  {
    ...
    mycontentPanel.setLayout(gbl_contentPanel);
    {
      ...
      {
        JLabel lblFile
           = new JLabel("<html><center>HL7 v2 File Sort takes an input file containing raw HL7 v2 messages, sorts them, and produces a resulting output file. This application uses a large merge-sort, so it can handle very large files.</center></html>");
    ...
    updateButtonStates();
    WindowUtils.fitAndCenterInScreen(this, 0.8f, 0.8f);
    WindowUtils.addEscapeListener(this, (e) -> hide());
  }

  public void addMessage(String theMsg)
  {
    myOutputListModel.add(theMsg);
    EventQueue.invokeLater(() -> myOutputScrollPane.getVerticalScrollBar()
       .setValue(myOutputScrollPane.getVerticalScrollBar().getMaximum()));
  }

  private void chooseFile(JTextField textField)
  {
    File currentDir = lastUsedCurrentDir;

    try
    {
      File tmp = new File(textField.getText());
      if(tmp.isFile())
      {
        currentDir = tmp.getParentFile();
      }
    }
    catch(Exception e2)
    {
    ...
    try
    {
      File temp = FileDialogHelper.chooseFile(this, "Choose file HL7", currentDir, true);
      if(temp != null)
      {
        lastUsedCurrentDir = temp.getParentFile();
        textField.setText(temp.getAbsolutePath());
      }
    }
    catch(Exception exception)
    {
      ErrorDialog.showError(this, exception);
    }
  }
  ...
  public IProgressCallback provideProgressCallback()
  {
    return new IProgressCallback()
    {
      public void activityStarted()
      {
        myRunning = true;
        myCancelled = false;
        EventQueue.invokeLater(() -> updateButtonStates());
      }
      ...
      public void progressUpdate(final double theProgress)
         throws OperationCancelRequestedException
      {
        EventQueue.invokeLater(() -> myProgressBar.setValue((int) (myProgressBar.getMaximum() * theProgress)));
        if(myCancelled)
  ...
  private void updateButtonStates()
  {
    ...
    boolean haveFiles
       = (StringUtils.isNotBlank(myInputFileText.getText()) && StringUtils.isNotBlank(myOutputFileText.getText()));
