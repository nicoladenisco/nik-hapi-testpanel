...
public class OutboundConnection extends AbstractConnection implements IDestroyable
{
  ...
  private class ConnectionMonitorThread extends Thread
  {
       ...
       throws LLPException
    {
      ...
      String desc = OutboundConnection.this.isNameIsExplicitlySet()
                       ? OutboundConnection.this.getName() + " (" + OutboundConnection.this.createDescription() + ")"
                       : OutboundConnection.this.createDescription();
      ...
      while(myConnectionMonitorThread == this)
      {
        if(getStatus() == StatusEnum.TRYING_TO_START)
        {
          ...
          try
          {
            ...
            if(isDualPort())
            {
              connection = ConnectionHub.getInstance()
                 .attach(getHost(), getOutgoingPort(), getIncomingOrSinglePort(), myParser, llpClass, tls, socketFactory);
            }
            else
            {
              connection = ConnectionHub.getInstance()
                 .attach(getHost(), getIncomingOrSinglePort(), myParser, llpClass, tls, socketFactory);
            }
          ...
          catch(HL7Exception e)
          {
            ...
            addActivity(new ActivityInfoError(new Date(),
               "Failed to connect to " + createDescription() + " - " + ex.getMessage()));
          }
          ...
          CommonFileUtils.safeSleep(1000);
        }

        CommonFileUtils.safeSleep(250);
      }
    }
  ...
  public class MessageSenderThread extends Thread
  {
    ...
    public void run()
    {
      EventQueue.invokeLater(() -> myTransmissionCallback.activityStarted());
      ...
      {
        EventQueue.invokeLater(() -> myTransmissionCallback.activityStopped());
      ...
      for(AbstractMessage<?> abstractMessage : myMessages.getMessages())
      {
        ...
        if(abstractMessage instanceof Hl7V2MessageBase)
        {
          ...
          try
          {
            ...
            addActivity(new ActivityOutgoingMessage(new Date(), getEncoding(),
               myParser.encode(msg), EncodingCharacters.getInstance(msg)));
            ...
            addActivity(new ActivityIncomingMessage(new Date(), getEncoding(),
               myParser.encode(response), EncodingCharacters.getInstance(response)));
          }
          catch(HL7Exception e)
          {
